...
export async function POST({ request, params }) {
    ...
    const existingMembership = await Member.findOne({ placeId: invite.placeId, userId: user._id });

    if (existingMembership) {
        return new Response("You are already a member of this server", { status: 409 });
    }
