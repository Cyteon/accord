...
export async function POST({ request, params }) {
    ...
    const members = await Member.find({ userId: user._id, placeId: { $in: placesShouldBeIn } });

    if (!members) {
        return Response.json({ error: "You cannot access these channels" }, { status: 403 });
    }

    if (members.length != placesShouldBeIn.length) {
        return Response.json({ error: "You cannot all or some access these channels" }, { status: 403 });
    }
